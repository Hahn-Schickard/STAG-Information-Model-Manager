...
struct FullMetaInfoTests : public TestWithParam<MetaInfoTestParam> {
  FullMetaInfoTests() : expected(GetParam()) {
    tested = make_shared<FullMetaInfo>(
        expected.id, BuildInfo{expected.name, expected.description});
  }

  MetaInfoTestParam expected;
  FullMetaInfoPtr tested;
};

TEST_P(FullMetaInfoTests, returnsId) { EXPECT_EQ(tested->id(), expected.id); }

TEST_P(FullMetaInfoTests, returnsName) {
  EXPECT_EQ(tested->name(), expected.name);
}

TEST_P(FullMetaInfoTests, returnsDescription) {
  EXPECT_EQ(tested->description(), expected.description);
}

INSTANTIATE_TEST_SUITE_P(FullMetaInfoTestsValues,
    FullMetaInfoTests,
    ...
    [](const TestParamInfo<FullMetaInfoTests::ParamType>& info) {
